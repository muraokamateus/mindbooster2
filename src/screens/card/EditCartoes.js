...
export default function AddCartoes(props) {
    console.log(props.route.params.idItemCartao);


    const [frente, setFrente] = useState(props.route.params.iFrente)
    const [verso, setVerso] = useState(props.route.params.iVerso)
    const [idCartao, setIdCartao] = useState(props.route.params.idItemCartao)
    ...
    const changeCartoes=(id)=>{ 
        console.log(id)
        const cartoesRef = doc(db,"cartoes",id)
        
        updateDoc(cartoesRef,{
            frente,
            verso,
        })
    }
